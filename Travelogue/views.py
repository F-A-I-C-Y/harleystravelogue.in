...
db = MySQLdb.connect('localhost','root','','harleys_travelogue')
c = db.cursor()
...
def Customer_Registration(request):
    if request.POST:
        ...
        type= "Customer"
        qry="insert into cust_reg(cname,address,gender,age,district,email,mobile,password) values('"+ cname +"','"+ address +"','"+ gender +"','"+ age +"','"+ district +"','"+ email +"','"+ mobile +"','"+ password +"')"
        qr ="insert into login values('"+ email +"','"+ password +"','"+ type +"')"
        c.execute(qry)
        c.execute(qr)
        # subject = 'Welcome to Harleys Travelogue'
        # message = f'Hi {cname}, thank you for being one of us. Continue with us for more experience.'
...
def Club_Registration(request):
    if request.POST:
        ...
        status = "Registered"
        type = "Club"
        qry = "insert into club_reg(clname,district,address,phone,email,password,status) values('"+ cname +"','"+ district +"','"+ address +"','"+ mobile +"','"+ email +"','"+ password +"','"+ status +"')"
        qr ="insert into login values('"+ email +"','"+ password +"','"+ type +"')"
        c.execute(qry)
        c.execute(qr)
        db.commit()
...
def Shop_Registration(request):
    if request.POST:
        ...
        mobile = request.POST.get("mobile")
        password = request.POST.get("Password")
        qry = "insert into shop_reg(shop,district,address,phone,email,password,status) values('"+ cname +"','"+ district +"','"+ address +"','"+ mobile +"','"+ email +"','"+ password +"','Registered')"
...
def Club_Home(request):
    ...
    c.execute("SELECT clname from club_reg where clid='"+str(clid)+"'")
    data = c.fetchall()
    # clid = request.session['id']
    # data=""
    # if request.session['clid']:   
    #     c.execute(" select clid, clname from club_reg where clid='"+str(clid)+"'")
    #     data=c.fetchall()
    return render(request,'Club Home.html',{"data":data})
...
def Admin_View_Club(request):
    c.execute("SELECT * from club_reg")
    data = c.fetchall()
    if request.GET:
...
def Club_Add_Notification(request):
    ...
    data=c.fetchall() 
    return render(request,"Club Add Notification.html",{"data":data, "data1":data1,"msg":msg})
...
def Club_View_Request(request):
    clid = request.session['cid']
    c.execute("SELECT clubjoin_req.*, cust_reg.*,club_reg.clid,club_reg.clname FROM clubjoin_req INNER JOIN cust_reg ON clubjoin_req.uid=cust_reg.cid inner join club_reg on club_reg.clid=clubjoin_req.clid where club_reg.clid = '"+str(clid)+"' ")
    data = c.fetchall()
    if request.GET:
        cl = request.GET.get('id')
        st = request.GET.get('st')
...
def Club_Add_Vehicle(request):
    ...
    data=c.fetchall() 
    return render(request,'Club Add Vehicle.html',{"data":data,"data1":data1,"msg":msg})
...
def Club_View_Booking(request):
    ...
    if request.GET:
        ...
        if st =='Accept':
            ...
            # sendsms(mo,msg)
            return HttpResponseRedirect("/ClubViewBooking/")
    return render(request,"Club View Booking.html",{"data":data, "msg":msg})
...
def Customer_View_Profile(request):
    ...
    if request.session['uid']:   
        c.execute(" select cname, address, email, mobile from cust_reg where cid='"+str(uid)+"'")
        data=c.fetchall()
        if request.POST:
            Address=request.POST.get("Address")
            ...
            db.commit()
            msg = "Profile updated"
    return render (request,"Customer View Profile.html",{"data":data, "msg":msg})
...
def CustomerViewCart(Request):
    ...
    f="select * from cust_reg where cid='"+str(cid)+"'"
    c.execute(f)
    data4=c.fetchall()
    ...
    return render(Request,'CustomerViewCart.html',{"data":data, "data1":data1[0],"data2":data2[0], "data4":data4})
...
def payment(request):
    ...
    if request.POST:
        msg="Booking Has Successfully Completed"
        return HttpResponseRedirect("/payment1?msg="+msg)
    return render (request,"payment.html",{"msg":msg,"amount":amount})

def payment1(request):
    if(request.POST):
        c.execute("select * from cart where cid = '"+str(uid)+"'")
        data3 = c.fetchall()
        for d3 in data3:
            custid = d3[1]
            proid = d3[2]
...
def reciept(request):
    ...
    uid=request.session['uid']
    amount=request.session["pay"]
    s="select * from cart inner join products on cart.pid = products.product_id where cart.cid = '"+str(uid)+"'"
    c.execute(s)
    data=c.fetchall()
    t="select count(*) from cart where cid = '"+str(uid)+"'"
    c.execute(t)
    data1=c.fetchone()
    u="select sum(price) from cart where cid = '"+str(uid)+"'"
    c.execute(u)
    data2=c.fetchone()
    totalamount = data2[0]
    tot = totalamount
    ...
    return render(request,"reciept.html",{"msg":msg,"amount":amount,"data":data, "data1":data1[0],"data2":data2[0]})    
...
def ShopUpdateProduct(request):
    ...
    if request.POST:
            ...
            c.execute("update products set product_title = '"+str(title)+"', product_price = '"+str(price)+"', quantity = '"+str(qty)+"', product_image = '"+str(image)+"' where product_id = '"+str(pid)+"'")
            db.commit()
            return HttpResponseRedirect("/ShopViewMyProduct/")
